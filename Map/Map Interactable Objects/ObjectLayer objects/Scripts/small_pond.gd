...
## List of factions that make this object appear as dry
const DRIED_FACTIONS = [
	GlobalDefs.Faction.Necropolis,
	GlobalDefs.Faction.DarkForces,
]
const FULL_ATLAS_REGION = Rect2(  0.0, 39.0, 39.0, 39.0 )
const DRY_ATLAS_REGION =  Rect2( 39.0, 39.0, 39.0, 39.0 )
...
func _make_dry() -> void:
	if _is_dry: return
	object_name = "Dried pond"
	_is_dry = true
	(sprite.texture as AtlasTexture).region = DRY_ATLAS_REGION

func _make_full() -> void:
	if not _is_dry: return
	object_name = "Pond"
	_is_dry = false
	(sprite.texture as AtlasTexture).region = FULL_ATLAS_REGION

func _update_state() -> void:
	if tile_data.tile_owner == null: return
	if tile_data.tile_owner in DRIED_FACTIONS: _make_dry()
	else: _make_full()

func _check_claimed_tile(tile: MapTileData, prev: MapFaction) -> void:
	super._check_claimed_tile(tile, prev)
...
func _initialize() -> void:
	(sprite.texture as AtlasTexture).region = \
		DRY_ATLAS_REGION if _is_dry else FULL_ATLAS_REGION
	object_name = "Dried pond" if _is_dry else "Pond"
	_update_state()
