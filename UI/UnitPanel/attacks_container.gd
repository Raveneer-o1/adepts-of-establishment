...
const ATTACK_PANELS_CONTAINER = preload("uid://cl5xq0emjsk46")
...
func fill_attacks(a: Array) -> void:
	for ch in h_box_container.get_children():
		if ch is UI_UnitPanel_AttackPanel_Container: ch.queue_free()
	for atk: Variant in a:
		if atk is not UnitAttack:
			if atk is not UnitAttackData:
				push_error("Unexpected type")
				continue
		var obj: UI_UnitPanel_AttackPanel_Container = ATTACK_PANELS_CONTAINER.instantiate()
